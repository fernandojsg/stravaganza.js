...
export class DebugOverlay {
  ...
  _onMouseUp() {
    if (this._dragging) {
      this._dragging = false;
      // Final seek to ensure position is firmly set before resuming.
      // The last mousemove may have been coalesced by the browser.
      if (this.onSeek) this.onSeek(this._lastDragTimeMs);
  ...
  _createVideoOverlay() {
    ...
    // Load video as blob URL so seeking works without HTTP range request support
    fetch('odyssey.mp4')
      .then(r => r.blob())
      .then(blob => { video.src = URL.createObjectURL(blob); })
      .catch(() => { video.src = 'odyssey.mp4'; /* fallback */ });
  ...
  _syncVideo(timeMs, isPlaying) {
    ...
    // Use tighter threshold when paused/scrubbing (0.05s) vs playing (0.3s)
    const threshold = isPlaying ? 0.3 : 0.05;
    if (Math.abs(this._videoEl.currentTime - timeSec) > threshold) {
